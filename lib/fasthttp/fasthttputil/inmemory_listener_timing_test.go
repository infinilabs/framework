...
func benchmark(b *testing.B, h fasthttp2.RequestHandler, isTLS bool) {
	bc := &benchConfig{
		IsTLS: isTLS,
	}
	benchmarkExt(b, h, bc)
...
func benchmarkExt(b *testing.B, h fasthttp2.RequestHandler, bc *benchConfig) {
	...
	go func() {
		...
		if err := fasthttp2.Serve(serverLn, h); err != nil {
...
func streamingHandler(ctx *fasthttp2.RequestCtx) {
	ctx.WriteString("foobar") //nolint:errcheck
}

func handshakeHandler(ctx *fasthttp2.RequestCtx) {
...
func runRequests(b *testing.B, pb *testing.PB, c *fasthttp2.HostClient) {
	...
	for pb.Next() {
		...
		if resp.StatusCode() != fasthttp2.StatusOK {
			b.Fatalf("unexpected status code: %d. Expecting %d", resp.StatusCode(), fasthttp2.StatusOK)
		}
	}
}
