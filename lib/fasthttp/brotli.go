...
func acquireStacklessBrotliWriter(w io.Writer, level int) stackless2.Writer {
...
func releaseStacklessBrotliWriter(sw stackless2.Writer, level int) {
	sw.Close()
	nLevel := normalizeBrotliCompressLevel(level)
	p := stacklessBrotliWriterPoolMap[nLevel]
	p.Put(sw)
...
var stacklessWriteBrotli = stackless2.NewFunc(nonblockingWriteBrotli)
