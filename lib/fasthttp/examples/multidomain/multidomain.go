...
var domains = make(map[string]fasthttp2.RequestHandler)

func main() {
	server := &fasthttp2.Server{
		...
		// > EOF
		//
		Handler: func(ctx *fasthttp2.RequestCtx) {
	...
	domains["localhost:8080"] = func(ctx *fasthttp2.RequestCtx) {
	...
	domains["127.0.0.1:8080"] = func(ctx *fasthttp2.RequestCtx) {
