...
func Factory(c *config.Config, defaultPath string, strictPerms bool) (Keystore, error) {
	cfg := defaultConfig()
...
// NewFileKeystoreWithStrictPerms returns an new File based keystore or an error, currently users cannot set their
// own password on the keystore, the default password will be an empty string. When the keystore
// is initialized the secrets are automatically loaded into memory.
...
func (k *FileKeystore) GetConfig() (*config.Config, error) {
	k.RLock()
	defer k.RUnlock()
