...
func (r *Router) tryRedirect(ctx *fasthttp.RequestCtx, tree *radix.Tree, tsr bool, method, path string) bool {
	...
	if tsr && r.RedirectTrailingSlash {
		...
		queryBuf := ctx.PhantomURI().QueryString()
		if len(queryBuf) > 0 {
	...
	if r.RedirectFixedPath {
		phantomURI := ctx.Request.PhantomURI()
		path := strconv.B2S(phantomURI.Path())
		...
		if found {
			queryBuf := phantomURI.QueryString()
			if len(queryBuf) > 0 {
				uri.WriteByte(questionMark)
				uri.Write(queryBuf)
			}
...
func (r *Router) Handler(ctx *fasthttp.RequestCtx) {
	...
	var path = strconv.B2S(ctx.Request.PhantomURI().PathOriginal())
