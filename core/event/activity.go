...
type Activity struct {
	...
	Changelog interface{} `json:"changelog,omitempty" elastic_mapping:"changelog:{type: flattened }"`
	Fields    util.MapStr   `json:"payload" elastic_mapping:"payload:{type: object,enabled:false }"`
}

type ActivityMetadata struct {
	Labels util.MapStr `json:"labels,omitempty"  elastic_mapping:"labels:{type: flattened }"`
