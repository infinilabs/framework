...
// is less than MaxFileSize megabytes, lumberjack will open and append to that file.
// If the file exists and its size is >= MaxFileSize megabytes, the file is renamed
// by putting the current time in a timestamp in the name immediately before the
// file's extension (or the end of the filename if there's no extension). A new
// log file is then created using original filename.
//
// Whenever a write would cause the current log file exceed MaxFileSize megabytes,
...
// number equal to MaxRotationCount (or all of them if MaxRotationCount is 0).  Any files
// with an encoded timestamp older than MaxFileAge days are deleted, regardless of
// MaxRotationCount.  Note that the time encoded in the timestamp is the rotation
// time, which may differ from the last time that file was written to.
//
// If MaxRotationCount and MaxFileAge are both 0, no old log files will be deleted.
type RotateWriter struct {
	...
	MaxFileSize int `json:"maxsize" yaml:"maxsize"`

	// MaxFileAge is the maximum number of days to retain old log files based on the
	// timestamp encoded in their filename.  Note that a day is defined as 24
	// hours and may not exactly correspond to calendar days due to daylight
	// savings, leap seconds, etc. The default is not to remove old log files
	// based on age.
	MaxFileAge int `json:"maxage" yaml:"maxage"`

	// MaxRotationCount is the maximum number of old log files to retain.  The default
	// is to retain all old log files (though MaxFileAge may still cause them to get
	// deleted.)
	MaxRotationCount int `json:"maxbackups" yaml:"maxbackups"`
...
// than MaxFileSize, the file is closed, renamed to include a timestamp of the
...
// would not put it over MaxFileSize.  If there is no such file or the write would
// put it over the MaxFileSize, a new file is created.
...
func (l *RotateWriter) filename() string {
	...
	name := filepath.Base(os.Args[0]) + "-rotated.log"
...
func (l *RotateWriter) millRunOnce() error {
	...
	var compress, remove []fileInfo

	if l.MaxRotationCount > 0 && l.MaxRotationCount < len(files) {
		...
		for _, f := range files {
			...
			if len(preserved) > l.MaxRotationCount {
...
func (l *RotateWriter) oldLogFiles() ([]fileInfo, error) {
	...
	for _, f := range files {
		...
		if t, err := l.timeFromName(f.Name(), prefix, ext); err == nil {
			logFiles = append(logFiles, fileInfo{t, f})
			continue
		}
...
// fileInfo is a convenience struct to return the filename and its embedded
// timestamp.
